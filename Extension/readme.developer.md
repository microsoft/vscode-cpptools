...
`npm run bootstrap` installs Yarn `1.22.22` from the internal npm feed using the pinned `.yarn-bootstrap/package-lock.json`, exposes the verified local package globally, and then runs `yarn install --frozen-lockfile`.
