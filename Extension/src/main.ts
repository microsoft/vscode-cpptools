...
export async function activate(context: vscode.ExtensionContext): Promise<CppToolsApi & CppToolsExtension> {
    ...
    await makeBinariesExecutable(info);
...
async function makeBinariesExecutable(info: PlatformInformation): Promise<void> {
...
function sendTelemetry(info: PlatformInformation): boolean {
    ...
    const success: boolean = true;//!installBlob.hasError;

    telemetryProperties['success'] = success.toString();
    //telemetryProperties['type'] = installBlob.type === InstallationType.Online ? "online" : "offline";
