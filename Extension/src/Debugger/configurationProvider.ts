...
export class DebugConfigurationProvider implements vscode.DebugConfigurationProvider {
    ...
    async provideDebugConfigurations(folder?: vscode.WorkspaceFolder, token?: vscode.CancellationToken): Promise<CppDebugConfiguration[]> {
        ...
        const items: ConfigMenu[] = configs.map<ConfigMenu>(config => {
            ...
            if (isDebugLaunchStr(menuItem.label)) {
                menuItem.label = vscode.l10n.t("Default Configuration");
            }
            return menuItem;
        });

        const selection: ConfigMenu | undefined = await vscode.window.showQuickPick(this.localizeConfigDetail(items), { placeHolder: vscode.l10n.t("Select a configuration") });
        if (!selection) {
    ...
    async resolveDebugConfigurationWithSubstitutedVariables(folder: vscode.WorkspaceFolder | undefined, config: CppDebugConfiguration, token?: vscode.CancellationToken): Promise<CppDebugConfiguration | null | undefined> {
        ...
        if (config.type === DebuggerType.cppvsdbg) {
            ...
            if (os.platform() !== 'win32') {
                void logger.getOutputChannelLogger().showWarningMessage(vscode.l10n.t("Debugger of type: '{0}' is only available on Windows. Use type: '{1}' on the current OS platform.", "cppvsdbg", "cppdbg"));
                return undefined; // Abort debugging silently.
            }

            // Handle legacy 'externalConsole' bool and convert to console: "externalTerminal"
            if (config.hasOwnProperty("externalConsole")) {
                void logger.getOutputChannelLogger().showWarningMessage(vscode.l10n.t("The key '{0}' is deprecated. Please use '{1}' instead.", "externalConsole", "console"));
        ...
        if (os.platform() === 'darwin' && // Check for macOS
            ...
            if (!frameworkPath) {
                ...
                const LLDBFrameworkMissingMessage: string = vscode.l10n.t("Unable to locate 'LLDB.framework' for lldb-mi. Please install XCode or XCode Command Line Tools.");
        ...
        if (config.logging?.engineLogging) {
            ...
            outputChannel.appendLine(vscode.l10n.t("Launch configuration:"));
        ...
        if (config.deploySteps && config.deploySteps.length !== 0) {
            ...
            if ((util.isNumber(codeVersion[0]) && codeVersion[0] < 1) || (util.isNumber(codeVersion[0]) && codeVersion[0] === 1 && util.isNumber(codeVersion[1]) && codeVersion[1] < 69)) {
                void logger.getOutputChannelLogger().showErrorMessage(vscode.l10n.t("'deploySteps' require VS Code 1.69+."));
                return undefined;
            }
    ...
    async provideDebugConfigurationsForType(type: DebuggerType, folder?: vscode.WorkspaceFolder, _token?: vscode.CancellationToken): Promise<CppDebugConfiguration[]> {
        ...
        if (buildTasks.length !== 0) {
            configs = (await Promise.all(buildTasks.map<Promise<CppDebugConfiguration | undefined>>(async task => {
                ...
                if (!compilerPathExists) {
                    logger.getOutputChannelLogger().appendLine(vscode.l10n.t("Unable to find {0}. {1} task is ignored.", compilerPath, definition.label));
                }
                ...
                newConfig.detail = vscode.l10n.t("preLaunchTask: {0}", task.name);
                ...
                if (platformInfo.platform !== "darwin") {
                    ...
                    if (await util.checkFileExists(debuggerPath)) {
                        ...
                        if (!isWindows && await util.checkFileExists(usrDebuggerPath)) {
                            ...
                            logger.getOutputChannelLogger().appendLine(vscode.l10n.t("Unable to find the {0} debugger. The debug configuration for {1} is ignored.", `\"${debuggerName}\"`, compilerName));
                            return undefined;
                        }
                    }
                }
        ...
        const existingConfigs: CppDebugConfiguration[] | undefined = this.getLaunchConfigs(folder, type)?.map(config => {
            if (!config.detail && config.preLaunchTask) {
                config.detail = vscode.l10n.t("preLaunchTask: {0}", config.preLaunchTask);
            }
    ...
    private buildAndDebugActiveFileStr(): string {
        return `${vscode.l10n.t('build and debug active file')}`;
    }
    ...
    private async showErrorIfClNotAvailable(_configurationLabel: string): Promise<boolean> {
        ...
        const applyButton = vscode.l10n.t("Apply Developer Environment");
        const applyMessage = vscode.l10n.t({ message: "{0} requires the Visual Studio developer environment.", args: [`cl.exe ${this.buildAndDebugActiveFileStr()}`], comment: ["{0} is a command option in a menu."] });
        const updateButton = vscode.l10n.t("Update Developer Environment");
        const updateMessage = vscode.l10n.t({ message: "{0} requires the Visual Studio developer environment to be updated.", args: [`cl.exe ${this.buildAndDebugActiveFileStr()}`], comment: ["{0} is a command option in a menu."] });
        ...
        const notAppliedMessage = vscode.l10n.t("The source code could not be built because the Visual Studio developer environment was not applied.");
        const notFoundMessage = vscode.l10n.t("The source code could not be built because the Visual C++ compiler could not be found.");
    ...
    private getLLDBFrameworkPath(): string | undefined {
        ...
        outputChannel.appendLine(vscode.l10n.t("Missing dependency '{0}' for lldb-mi executable.", LLDBFramework));
        outputChannel.appendLine(vscode.l10n.t("Searched in:"));
        searchPaths.forEach(searchPath => {
            outputChannel.appendLine(`\t${searchPath}`);
        });
        const xcodeCLIInstallCmd: string = "xcode-select --install";
        outputChannel.appendLine(vscode.l10n.t("To resolve this issue, either install XCode through the Apple App Store or install the XCode Command Line Tools by running '{0}' in a Terminal window.", xcodeCLIInstallCmd));
        logger.showOutputChannel();
    ...
    private resolveEnvFile(config: CppDebugConfiguration, folder?: vscode.WorkspaceFolder): void {
        if (config.envFile) {
            ...
            try {
                ...
                throw new Error(vscode.l10n.t("Failed to use {0}. Reason: {1}", "envFile", e.message));
            }
        }
    }
        ...
        if (config.sourceFileMap) {
            for (const sourceFileMapSource of Object.keys(config.sourceFileMap)) {
                ...
                if (sourceFileMapSource !== newSourceFileMapSource) {
                    message = "\t" + vscode.l10n.t("Replacing {0} '{1}' with '{2}'.", "sourcePath", sourceFileMapSource, newSourceFileMapSource);
                ...
                if (util.isString(sourceFileMapTarget)) {
                    ...
                    if (sourceFileMapTarget !== newSourceFileMapTarget) {
                        ...
                        message += message ? ' ' : '\t';
                        message += vscode.l10n.t("Replacing {0} '{1}' with '{2}'.", "targetPath", sourceFileMapTarget, newSourceFileMapTarget);
                    ...
                    if (sourceFileMapTarget !== newSourceFileMapTarget) {
                        // Add a space if source was changed, else just tab the target message.
                        message += message ? ' ' : '\t';
                        message += vscode.l10n.t("Replacing {0} '{1}' with '{2}'.", "editorPath", sourceFileMapTarget, newSourceFileMapTarget["editorPath"]);
                        target = newSourceFileMapTarget;
                    }
                }
            ...
            if (messages.length > 0) {
                logger.getOutputChannel().appendLine(vscode.l10n.t("Resolving variables in {0}...", "sourceFileMap"));
    ...
    private static async showFileWarningAsync(message: string, fileName: string): Promise<void> {
        const openItem: vscode.MessageItem = { title: vscode.l10n.t("Open {0}", "envFile") };
        ...
        items.map((item: ConfigMenu) => {
            switch (item.detail) {
                case TaskStatus.recentlyUsed: {
                    item.detail = vscode.l10n.t("Recently Used Task");
                    break;
                }
                case TaskStatus.configured: {
                    item.detail = vscode.l10n.t("Configured Task");
                    break;
                }
                case TaskStatus.detected: {
                    item.detail = vscode.l10n.t("Detected Task");
                    break;
                }
                default: {
                    break;
    ...
    private async selectConfiguration(textEditor: vscode.TextEditor, pickDefault: boolean = true, onlyWorkspaceFolder: boolean = false): Promise<CppDebugConfiguration | undefined> {
        ...
        if (!util.isCppOrCFile(textEditor.document.uri)) {
            void vscode.window.showErrorMessage(vscode.l10n.t('Cannot build and debug because the active file is not a C or C++ source file.'));
            return;
        }
        ...
        if (defaultConfig && defaultConfig.length === 1) {
            ...
            selection = await vscode.window.showQuickPick(this.localizeConfigDetail(sortedItems), {
                placeHolder: items.length === 0 ? vscode.l10n.t("No compiler found") : vscode.l10n.t("Select a debug configuration")
            });
        }
        ...
        switch (stepType) {
            case StepType.command: {
                ...
                if (step.args && !Array.isArray(step.args)) {
                    void logger.getOutputChannelLogger().showErrorMessage(vscode.l10n.t('"args" in command deploy step must be an array.'));
                    return false;
                }
            ...
            case StepType.rsync: {
                ...
                if (!step.files || !step.targetDir || !step.host) {
                    void logger.getOutputChannelLogger().showErrorMessage(vscode.l10n.t('"host", "files", and "targetDir" are required in {0} steps.', isScp ? 'SCP' : 'rsync'));
                    return false;
                }
                ...
                if (util.isString(step.files)) {
                    ...
                    void logger.getOutputChannelLogger().showErrorMessage(vscode.l10n.t('"files" must be a string or an array of strings in {0} steps.', isScp ? 'SCP' : 'rsync'));
                    return false;
                }
            ...
            case StepType.ssh: {
                if (!step.host || !step.command) {
                    void logger.getOutputChannelLogger().showErrorMessage(vscode.l10n.t('"host" and "command" are required for ssh steps.'));
                    return false;
                }
                const host: util.ISshHostInfo = util.isString(step.host) ? { hostName: step.host } : { hostName: step.host.hostName, user: step.host.user, port: step.host.port };
                const jumpHosts: util.ISshHostInfo[] = step.host.jumpHosts;
            ...
            case StepType.shell: {
                if (!step.command) {
                    void logger.getOutputChannelLogger().showErrorMessage(vscode.l10n.t('"command" is required for shell steps.'));
                    return false;
                }
            ...
            default: {
                logger.getOutputChannelLogger().appendLine(vscode.l10n.t('Deploy step type {0} is not supported.', step.type));
                return false;
            }
        }
        return true;
...
export class ConfigurationAssetProviderFactory {
    public static getConfigurationProvider(): IConfigurationAssetProvider {
        switch (os.platform()) {
            ...
            default:
                throw new Error(vscode.l10n.t("Unexpected OS type"));
        }
    }
}
...
class WindowsConfigurationProvider extends DefaultConfigurationProvider {
    ...
    private pipeProgram: string = "<" + vscode.l10n.t("full path to pipe program such as {0}", "plink.exe").replace(/"/g, '') + ">";
    private MIMode: string = 'gdb';
    private setupCommandsBlock: string = `"setupCommands": [
    {
        "description": "${vscode.l10n.t("Enable pretty-printing for {0}", "gdb").replace(/"/g, '')}",
        "text": "-enable-pretty-printing",
        "ignoreFailures": true
    },
    {
        "description": "${vscode.l10n.t("Set Disassembly Flavor to {0}", "Intel").replace(/"/g, '')}",
