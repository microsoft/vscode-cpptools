...
const errorNoContext = vscode.l10n.t('No context provided');
const errorNotWindows = vscode.l10n.t('The "Set Visual Studio Developer Environment" command is only available on Windows');
const errorNoVSFound = vscode.l10n.t('A Visual Studio installation with the C++ compiler was not found');
export const errorOperationCancelled = vscode.l10n.t('The operation was cancelled');
const errorNoHostsFound = vscode.l10n.t('No hosts found');
const configuringDevEnv = vscode.l10n.t('Configuring developer environment...');
const selectVSInstallation = vscode.l10n.t('Select a Visual Studio installation');
const advancedOptions = vscode.l10n.t('Advanced options...');
const advancedOptionsDescription = vscode.l10n.t('Select a specific host and target architecture, toolset version, etc.');
const selectToolsetVersion = vscode.l10n.t('Select a toolset version');
const selectHostTargetArch = vscode.l10n.t('Select a host and target architecture');
...
export async function setEnvironment(context?: vscode.ExtensionContext) {
    ...
    if (!vars || !vars['INCLUDE']) {
        throw new Error(vscode.l10n.t('Something went wrong: {0}', JSON.stringify(vars)));
    }
    ...
    context.environmentVariableCollection.description = vscode.l10n.t('{0} developer environment for {1}', arch, vsDisplayNameWithSuffix(vs));
...
async function chooseVSInstallation(installations: vswhere.Installation[]): Promise<vswhere.Installation | undefined> {
    const items: vscode.QuickPickItem[] = installations.map(installation => <vscode.QuickPickItem>{
        ...
        detail: vscode.l10n.t('Default environment for {0}', installation.catalog.productDisplayVersion)
    });
    items.push({
...
async function setOptions(compiler: Compiler): Promise<void> {
    ...
    if (hostTargets.length > 1) {
        const items = hostTargets.map(ht => <vscode.QuickPickItem>{
            ...
            description: vscode.l10n.t('host = {0}, target = {1}', ht.host, ht.target)
        });
        const selection = await vscode.window.showQuickPick(items, {
