    ...
    private getTask: (compilerPath: string | util.IQuotedString, appendSourceToName: boolean, compilerArgs?: (string | util.IQuotedString)[], definition?: CppBuildTaskDefinition, detail?: string) => Task = (compilerPath: string | util.IQuotedString, appendSourceToName: boolean, compilerArgs?: (string | util.IQuotedString)[], definition?: CppBuildTaskDefinition, detail?: string) => {
        ...
        if (!definition) {
            ...
            const taskLabel: string = ((appendSourceToName && !compilerPathBase.startsWith(ext.configPrefix)) ?
                ext.configPrefix : "") + compilerPathBase + " " + vscode.l10n.t("build active file");
        ...
        task.detail = detail ? detail : vscode.l10n.t("compiler:") + " " + resolvedCompilerPathString;
    ...
    public async writeBuildTask(taskLabel: string, workspaceFolder?: WorkspaceFolder, setAsDefault: boolean = false): Promise<void> {
        ...
        if (!rawTasksJson.tasks.find((task: any) => task.label === selectedTask?.definition.label)) {
            const newTask: any = {
                ...
                detail: vscode.l10n.t("Task generated by Debugger.")
            };
...
class CustomBuildTaskTerminal implements Pseudoterminal {
    ...
    async openAsync(_initialDimensions: TerminalDimensions | undefined): Promise<void> {
        if (this.buildOptions.taskUsesActiveFile && !util.isCppOrCFile(window.activeTextEditor?.document.uri)) {
            this.writeEmitter.fire(vscode.l10n.t('Cannot build and debug because the active file is not a C or C++ source file.') + this.endOfLine);
        ...
        this.writeEmitter.fire(vscode.l10n.t("Starting build...") + this.endOfLine);
        await this.doBuild();
    }
            ...
            const spawnResult: number = await new Promise<number>(resolve => {
                if (child) {
                    ...
                    child.on('close', result => {
                        ...
                        if (result === null) {
                            this.writeEmitter.fire(vscode.l10n.t("Build run was terminated.") + this.endOfLine);
                            resolve(-1);
                        } else {
                            resolve(result);
                        }
    ...
    private printBuildSummary(error: string, stdout: string, stderr: string, spawnResult: number): number {
        if (spawnResult !== 0) {
            this.writeEmitter.fire(vscode.l10n.t("Build finished with error(s).") + this.endOfLine);
            return -1;
        }
        if (error || (!stdout && stderr && stderr.includes("error")) ||
            ...
            this.writeEmitter.fire(vscode.l10n.t("Build finished with warning(s).") + this.endOfLine);
            return 0;
        } else {
            this.writeEmitter.fire(vscode.l10n.t("Build finished successfully.") + this.endOfLine);
            return 0;
        }
    }
}
