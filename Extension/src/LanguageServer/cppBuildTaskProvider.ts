...
import { CustomExecution, Disposable, Event, EventEmitter, l10n, ProcessExecution, Pseudoterminal, ShellExecution, Task, TaskDefinition, TaskEndEvent, TaskExecution, TaskGroup, TaskProvider, tasks, TaskScope, TerminalDimensions, TextEditor, window, workspace, WorkspaceFolder } from 'vscode';
...
export class CppBuildTaskProvider implements TaskProvider {
    ...
    private getTask: (compilerPath: string | util.IQuotedString, appendSourceToName: boolean, compilerArgs?: (string | util.IQuotedString)[], definition?: CppBuildTaskDefinition, detail?: string) => Task = (compilerPath: string | util.IQuotedString, appendSourceToName: boolean, compilerArgs?: (string | util.IQuotedString)[], definition?: CppBuildTaskDefinition, detail?: string) => {
        ...
        task.detail = detail ? detail : l10n.t("compiler:") + " " + resolvedCompilerPathString;
    ...
    public async writeBuildTask(taskLabel: string, workspaceFolder?: WorkspaceFolder, setAsDefault: boolean = false): Promise<void> {
        ...
        if (!rawTasksJson.tasks.find((task: any) => task.label === selectedTask?.definition.label)) {
            const newTask: any = {
                ...
                detail: l10n.t("Task generated by Debugger.")
            };
...
class CustomBuildTaskTerminal implements Pseudoterminal {
    ...
    async openAsync(_initialDimensions: TerminalDimensions | undefined): Promise<void> {
        if (this.buildOptions.taskUsesActiveFile && !util.isCppOrCFile(window.activeTextEditor?.document.uri)) {
            this.writeEmitter.fire(l10n.t('Cannot build and debug because the active file is not a C or C++ source file.') + this.endOfLine);
        ...
        this.writeEmitter.fire(l10n.t("Starting build...") + this.endOfLine);
        await this.doBuild();
    }
            ...
            const spawnResult: number = await new Promise<number>(resolve => {
                if (child) {
                    ...
                    child.on('close', result => {
                        ...
                        if (result === null) {
                            this.writeEmitter.fire(l10n.t("Build run was terminated.") + this.endOfLine);
                            resolve(-1);
                        } else {
                            resolve(result);
                        }
    ...
    private printBuildSummary(error: string, stdout: string, stderr: string, spawnResult: number): number {
        if (spawnResult !== 0) {
            this.writeEmitter.fire(l10n.t("Build finished with error(s).") + this.endOfLine);
            return -1;
        }
        if (error || (!stdout && stderr && stderr.includes("error")) ||
            ...
            this.writeEmitter.fire(l10n.t("Build finished with warning(s).") + this.endOfLine);
            return 0;
        } else {
            this.writeEmitter.fire(l10n.t("Build finished successfully.") + this.endOfLine);
            return 0;
        }
    }
}
