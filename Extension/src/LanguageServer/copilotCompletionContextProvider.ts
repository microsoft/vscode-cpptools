...
export enum CopilotCompletionContextFeatureFlag {
    None = 0,
    Instant,
    Deferred
}
...
export class CopilotCompletionContextProvider implements ContextResolver<CodeSnippet> {
    ...
    private async getCompletionContextWithCancellation(context: ResolveRequest, startTime: number, out: Logger,
        ...
        try {
            let featureFlag: CopilotCompletionContextFeatureFlag = await this.getFeatureFlag(context);
            const featureFlagFromSettings = new CppSettings().copilotCompletionFeatureFlag;
            if (featureFlagFromSettings) {
                featureFlag = CopilotCompletionContextFeatureFlag[featureFlagFromSettings as keyof typeof CopilotCompletionContextFeatureFlag] ?? featureFlag;
            }
            telemetry.addRequestMetadata(documentUri, caretOffset, context.completionId,
                context.documentContext.languageId, { featureFlag });
 ...
 snippetsCount:${copilotCompletionContext.snippets.length}`;
    ...
    private async fetchTimeBudgetFactor(context: ResolveRequest): Promise<number> {
        const CppToolsCopilotCompletionContextTimeBudget = "CppToolsCopilotCompletionContextTimeBudget";
        try {
            const budgetFactor = context.activeExperiments.get(CppToolsCopilotCompletionContextTimeBudget);
            return (budgetFactor as number) ?? CopilotCompletionContextProvider.defaultTimeBudgetFactor;
        } catch (e) {
            console.warn(`fetchTimeBudgetFactor(): error fetching ${CppToolsCopilotCompletionContextTimeBudget}, using default: `, e);
    ...
    private async fetchMaxDistanceToCaret(context: ResolveRequest): Promise<number> {
        const CppToolsCopilotCompletionContextMaxCaretDistance = "CppToolsCopilotCompletionContextMaxCaretDistance";
        try {
            const budgetFactor = context.activeExperiments.get(CppToolsCopilotCompletionContextMaxCaretDistance);
            return (budgetFactor as number) ?? CopilotCompletionContextProvider.defaultMaxCaretDistance;
        } catch (e) {
            console.warn(`fetchMaxDistanceToCaret(): error fetching ${CppToolsCopilotCompletionContextMaxCaretDistance}, using default: `, e);
            return CopilotCompletionContextProvider.defaultMaxCaretDistance;
        }
    }

    private async getFeatureFlagName(context: ResolveRequest): Promise<string | undefined> {
        const CppToolsCopilotCompletionContextFeatureFlag = "CppToolsCopilotCompletionContextFeatureFlag";
        try {
            const experimentName = context.activeExperiments.get(CppToolsCopilotCompletionContextFeatureFlag);
            return (experimentName as string) ?? undefined;
        } catch (e) {
            console.warn(`getExperimentName(): error fetching ${CppToolsCopilotCompletionContextFeatureFlag}: `, e);
            return undefined;
        }
    }

    private async getFeatureFlag(context: ResolveRequest): Promise<CopilotCompletionContextFeatureFlag> {
        const featureFlagName = await this.getFeatureFlagName(context);
        switch (featureFlagName) {
            case "instant": return CopilotCompletionContextFeatureFlag.Instant;
            case "deferred": return CopilotCompletionContextFeatureFlag.Deferred;
            default: return CopilotCompletionContextFeatureFlag.None;
        }
    }
    ...
    public async resolve(context: ResolveRequest, copilotCancel: vscode.CancellationToken): Promise<CodeSnippet[]> {
        ...
        telemetry.addRequestMetadata(context.documentContext.uri, context.documentContext.offset,
            context.completionId, context.documentContext.languageId, { timeBudgetFactor });
