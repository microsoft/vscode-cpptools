...
export class CppProperties {
    ...
    private validateIntelliSenseMode(configuration: Configuration): string {
        ...
        if (isValid) {
            ...
            return vscode.l10n.t("IntelliSense mode {0} is incompatible with compiler path.", configuration.intelliSenseMode);
        }
    }
    ...
    public handleConfigurationChange(): void {
        ...
        void this.timeOperation(() => this.updateServerOnFolderSettingsChange()).then(result => {
            getOutputChannelLogger().appendLineAtLevel(5, vscode.l10n.t("Processed c_cpp_properties.json in {0}s", result.duration));
        }).catch(logAndReturn.undefined);
    }
    ...
    private async ensurePropertiesFile(): Promise<void> {
        if (this.propertiesFile && await util.checkFileExists(this.propertiesFile.fsPath)) {
            ...
            try {
                ...
                const err: Error = errJS as Error;
                const failedToCreate: string = vscode.l10n.t('Failed to create "{0}"', this.configFolder);
    ...
    private parsePropertiesFile(): boolean {
        ...
        try {
            ...
            if (!newJson || !newJson.configurations || newJson.configurations.length === 0) {
                throw { message: vscode.l10n.t("Invalid configuration file. There must be at least one configuration present in the array.") };
            }
            ...
            if (this.configurationJson.version !== configVersion) {
                ...
                if (this.configurationJson.version === 3) {
                    ...
                    void vscode.window.showErrorMessage(vscode.l10n.t('Unknown version number found in c_cpp_properties.json. Some features may not work as expected.'));
                }
            }
            ...
            if (dirty) {
                try {
                    ...
                    void vscode.window.showWarningMessage(vscode.l10n.t('Attempt to update "{0}" failed (do you have write access?)', this.propertiesFile.fsPath));
                    success = false;
                }
            }
            ...
            const err: Error = errJS as Error;
            const failedToParse: string = vscode.l10n.t('Failed to parse "{0}"', this.propertiesFile.fsPath);
    ...
    public static validateCompilerPath(resolvedCompilerPath: string, rootUri?: vscode.Uri): util.CompilerPathAndArgs {
        ...
        if (!isCl && compilerPathAndArgs.compilerPath) {
            ...
            if (!pathExists) {
                ...
                const message: string = vscode.l10n.t("Path is not a file: {0}", resolvedCompilerPath);
                compilerPathErrors.push(message);
    ...
    private async getErrorsForConfigUI(configIndex: number): Promise<ConfigurationErrors> {
        ...
        try {
            ...
            errors.includePath = result ?? duration >= 10 ? vscode.l10n.t("The include path validation took {0}s to evaluate", duration) : undefined;
        } catch (e) {
            errors.includePath = vscode.l10n.t("Failed to resolve include path. Error: {0}", (e as Error).message);
        }
    ...
    private async validatePath(input: string | string[] | undefined, { isDirectory = true, assumeRelative = true, globPaths = false } = {}): Promise<string | undefined> {
        ...
        for (const p of paths) {
            ...
            if (!pathExists) {
                let message: string = vscode.l10n.t("Cannot find: {0}", resolvedPath);
                if (quotedPath) {
                    message += '. ' + vscode.l10n.t('Do not add extra quotes around paths.');
                }
                errors.push(message);
                continue;
            }
            ...
            if (isDirectory && !util.checkDirectoryExistsSync(resolvedPath)) {
                const message: string = vscode.l10n.t("Path is not a directory: {0}", resolvedPath);
                errors.push(message);
            } else if (!isDirectory && !util.checkFileExistsSync(resolvedPath)) {
                const message: string = vscode.l10n.t("Path is not a file: {0}", resolvedPath);
                errors.push(message);
            }
        }
    ...
    private isConfigNameUnique(configName: string): string | undefined {
        ...
        if (occurrences && occurrences > 1) {
            errorMsg = vscode.l10n.t("{0} is a duplicate. The configuration name should be unique.", configName);
        }
        return errorMsg;
    }
    ...
    private async handleSquigglesImpl(): Promise<void> {
        ...
        for (const [configName, allRanges] of configNames) {
            if (allRanges && allRanges.length > 1) {
                dupErrorMsg = vscode.l10n.t("{0} is a duplicate. The configuration name should be unique.", configName);
        ...
        if (!dotConfigPathExists) {
            dotConfigMessage = vscode.l10n.t("Cannot find: {0}", String(dotConfigPath));
            newSquiggleMetrics.PathNonExistent++;
        } else if (dotConfigPath && !util.checkFileExistsSync(dotConfigPath)) {
            dotConfigMessage = vscode.l10n.t("Path is not a file: {0}", dotConfigPath);
            ...
            try {
                ...
                if (duration > 10 && configMatches) {
                    ...
                    const diagnostic: vscode.Diagnostic = new vscode.Diagnostic(
                        ...
                        vscode.l10n.t("Path took {0}s to evaluate", duration),
                        vscode.DiagnosticSeverity.Warning);
                    diagnostics.push(diagnostic);
                }
            } catch (e) {
                ...
                if (configMatches) {
                    ...
                    const diagnostic: vscode.Diagnostic = new vscode.Diagnostic(
                        new vscode.Range(document.positionAt(curTextStartOffset + curOffset), document.positionAt(curTextStartOffset + endOffset)),
                        vscode.l10n.t("Failed to resolve path {0}. Error: {1}", curPath, (e as Error).message),
                        vscode.DiagnosticSeverity.Warning);
                    diagnostics.push(diagnostic);
                }
            }
            ...
            if (configMatches && !globPath) {
                ...
                for (const curMatch of configMatches) {
                    ...
                    if (!pathExists) {
                        ...
                        message = vscode.l10n.t("Cannot find: {0}", badPath);
                        if (incorrectExpandedPaths.some(p => p.match(/".*"/) !== null)) {
                            message += '.\n' + vscode.l10n.t('Do not add extra quotes around paths.');
                        }
                        newSquiggleMetrics.PathNonExistent++;
                    } else {
                        ...
                        if (curOffset >= forcedIncludeStart && curOffset <= forcedeIncludeEnd) {
                            if (expandedPaths.length > 1) {
                                message = vscode.l10n.t("Multiple paths are not allowed.");
                                ...
                                message = vscode.l10n.t("Path is not a file: {0}", expandedPaths[0]);
                                newSquiggleMetrics.PathNotAFile++;
                            }
                            ...
                            if (expandedPaths.length > 1) {
                                message = vscode.l10n.t("Multiple paths should be separate entries in an array.");
                            ...
                            if (mismatchedPaths.length > 1) {
                                ...
                                message = vscode.l10n.t("Paths are not directories: {0}", badPath);
                                newSquiggleMetrics.PathNotADirectory++;
                            } else if (mismatchedPaths.length === 1) {
                                badPath = `"${mismatchedPaths[0]}"`;
                                message = vscode.l10n.t("Path is not a directory: {0}", badPath);
                                newSquiggleMetrics.PathNotADirectory++;
                            } else {
                                continue;
                            }
                ...
                if (envMatches) {
                    ...
                    for (const curMatch of envMatches) {
                        ...
                        let message: string;
                        if (!pathExists) {
                            message = vscode.l10n.t("Cannot find: {0}", expandedPaths[0]);
                            newSquiggleMetrics.PathNonExistent++;
                            const diagnostic: vscode.Diagnostic = new vscode.Diagnostic(
