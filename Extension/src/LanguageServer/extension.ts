    ...
    PlatformInformation.GetPlatformInformation().then(info => {
        ...
        if (info.platform !== "linux" || info.architecture === "x86_64" || info.architecture === "arm") {
            ...
            const minimumSupportedVersionForInsidersUpgrades: PackageVersion =
                abTestSettings.getMinimumVSCodeVersion(new PackageVersion(util.packageJson.version));
...
function installVsix(vsixLocation: string): Thenable<void> {
    ...
    if (userVersion.isGreaterThan(lastVersionWithoutInstallExtensionCommand, "insiders")) {
    ...
    return PlatformInformation.GetPlatformInformation().then((platformInfo) => {
        ...
        if (userVersion.isGreaterThan(oldVersion, "insiders")) {
