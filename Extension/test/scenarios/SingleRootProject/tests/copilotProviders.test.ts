...
describe('registerRelatedFilesProvider', () => {
    ...
    const arrange = ({ cppToolsRelatedFilesApi, vscodeExtension, getIncludeFiles, chatContext, rootUri, flags }:
        ...
        mockCopilotApi.registerRelatedFilesProvider.callsFake((providerId: { extensionId: string; languageId: string }, callback: (uri: vscode.Uri, context: { flags: Record<string, unknown> }, cancellationToken: vscode.CancellationToken) => Promise<{ entries: vscode.Uri[]; traits?: CopilotTrait[] }>) => {
    ...
    it('should not add #cpp traits when ChatContext isn\'t available.', async () => {
        arrange({
            ...
            flags: { copilotcppProjectContext: true }
        });
        await moduleUnderTest.registerRelatedFilesProvider();
    ...
    it('should not add #cpp traits when copilotcppProjectContext flag is false.', async () => {
        arrange({
            cppToolsRelatedFilesApi: true,
            vscodeExtension: vscodeExtension,
            getIncludeFiles: { includedFiles: ['c:\\system\\include\\vector', 'c:\\system\\include\\string', 'C:\\src\\my_project\\foo.h'] },
            ...
            rootUri: vscode.Uri.file('C:\\src\\my_project'),
            flags: { copilotcppProjectContext: false }
        });
        await moduleUnderTest.registerRelatedFilesProvider();
    ...
    it('should add #cpp traits when copilotcppProjectContext flag is true.', async () => {
        arrange({
            cppToolsRelatedFilesApi: true,
            vscodeExtension: vscodeExtension,
            getIncludeFiles: { includedFiles: ['c:\\system\\include\\vector', 'c:\\system\\include\\string', 'C:\\src\\my_project\\foo.h'] },
    ...
    it('should exclude #cpp traits per copilotcppProjectContextExcludeTraits.', async () => {
        ...
        arrange({
            ...
            rootUri: vscode.Uri.file('C:\\src\\my_project'),
            flags: { copilotcppProjectContext: true, copilotcppProjectContextExcludeTraits: excludeTraits }
        });
        await moduleUnderTest.registerRelatedFilesProvider();
...
describe('registerRelatedFilesCommands', () => {
    ...
    const arrange = ({ cppToolsRelatedFilesApi, getIncludeFiles, chatContext, rootUri }:
        { cppToolsRelatedFilesApi: boolean, getIncludeFiles?: GetIncludesResult, chatContext?: ChatContextResult, rootUri?: vscode.Uri } =
        { cppToolsRelatedFilesApi: false, getIncludeFiles: undefined, chatContext: undefined, rootUri: undefined }
    ) => {
        getIsRelatedFilesApiEnabledStub.withArgs('CppToolsRelatedFilesApi').resolves(cppToolsRelatedFilesApi);
        activeClientStub.getIncludes.resolves(getIncludeFiles);
        sinon.stub(activeClientStub, 'RootUri').get(() => rootUri);
