...
class ActionBase {
    constructor(labels, milestoneName, milestoneId, ignoreLabels, ignoreMilestoneNames, ignoreMilestoneIds) {
        this.labels = labels;
    ...
    buildQuery(baseQuery) {
        ...
        if (this.labels) {
            ...
            for (const str in this.labelsSet) {
                if (str != "") {
        ...
        if (this.ignoreLabels) {
            if (this.ignoreLabels == "*" && !this.labels) { // only if unlabeled
                ...
                for (const str in this.ignoreLabelsSet) {
                    if (str != "") {
                        query = query.concat(` -label:"${str}"`);
                    }
                }
        ...
        if (this.ignoreMilestoneNames && this.ignoreMilestoneIds) {
            if (this.ignoreMilestoneNames == "*" && !this.milestoneName) { // only if no milestone
                ...
                for (const str in this.ignoreMilestoneNamesSet) {
                    if (str != "") {
    ...
    validateIssue(issue) {
        ...
        if (this.ignoreAllWithMilestones) {
            ...
            if (issue.milestoneId != null) {
                for (const str in this.ignoreMilestoneIdsSet) {
